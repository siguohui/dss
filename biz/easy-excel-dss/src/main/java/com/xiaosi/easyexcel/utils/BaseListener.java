...
import com.xiaosi.easyexcel.annotation.ExcelPropertyCheck;
...
public class BaseListener<T> implements ReadListener<T> {
        ...
        for (Field declaredField : declaredFields) {
            try {
                ...
                if(true ==declaredField.getAnnotation(ExcelPropertyCheck.class).checkDict()
                        && org.apache.commons.lang.StringUtils.isNotEmpty(declaredField.getAnnotation(ExcelPropertyCheck.class).dictName())){
    ...
    private String check(String str, Integer checkType) {
        if(org.apache.commons.lang.StringUtils.isEmpty(str) || "null".equals(str)){
            return "";
        }
        switch (checkType) {
            case 0:
